...
/*public*/
int readShaderSource(char *fileName, GLchar **vertexShader, GLchar **fragmentShader, GLchar **geometryShader)
{
	...
	gSize = shaderSize(fileName, EGeometryShader);

    if ((vSize == -1) || (fSize == -1 || (gSize == -1)))
    {
	...
	*geometryShader = (GLchar*)malloc(gSize);
	...
	if (!readShader(fileName, EGeometryShader, *geometryShader, vSize))
	{
		printf("Cannot read the file %s.geom\n", fileName);
		return 0;
	}
...
GLuint SetupGLSL(char *fileName){
	...
	if (geometryShaderObject == 0) {  // error checking 
		printf(" Error creating geometry shader object.\n");
		exit(1);
	}
	else printf(" Succeeded creating geometry shader object.\n");
	...
	readShaderSource(fileName, &vertexShaderSource, &fragmentShaderSource, &geometryShaderSource); 
	...
	glGetShaderiv(geometryShaderObject, GL_COMPILE_STATUS, &result);
	if (result == GL_FALSE) {
		printf(" GEOMETRY shader compilation failed!\n");
		GLint logLen;
		glGetShaderiv(geometryShaderObject, GL_INFO_LOG_LENGTH, &logLen);
		if (logLen > 0) {
			char *log = (char*)malloc(logLen);
			GLsizei written;
			glGetShaderInfoLog(geometryShaderObject, logLen, &written, log);
			printf("Sahder log: \n %s", log);
			free(log);
		}
	}
